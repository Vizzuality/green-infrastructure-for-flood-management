...
(function (PruneCluster_1) {
  ...
  const maxHashCodeValue = Math.pow(2, 53) - 1;
  const Marker = (function (_super) {
  ...
  const Cluster = (function (_super) {
    ...
    Cluster.prototype.ComputeBounds = function (cluster) {
      const proj = cluster.Project(this.position.lat, this.position.lng);
    ...
    Cluster.prototype.ApplyCluster = function (newCluster) {
      ...
      for (const category in newCluster.stats) {
  ...
  const PruneCluster = (function () {
    ...
    PruneCluster.prototype._sortMarkers = function () {
      ...
      if (this._nbChanges && !shouldUseInsertionSort(length, this._nbChanges)) {
        this._markers.sort((a, b) => { return a.position.lng - b.position.lng; });
      } else {
    ...
    PruneCluster.prototype.ProcessView = function (bounds) {
      ...
      const firstIndex = this._indexLowerBoundLng(extendedBounds.minLng);
      let markers = this._markers,
        clusters = this._clusters;
      const workingClusterList = clusters.slice(0);
    ...
    PruneCluster.prototype.FindMarkersInArea = function (area) {
      let aMinLat = area.minLat,
        ...
        result = [];
      const firstIndex = this._indexLowerBoundLng(aMinLng);
...
const PruneClusterForLeaflet = (L.Layer ? L.Layer : L.Class).extend({
  initialize(size, clusterMargin) {
    const _this = this;
  ...
  BuildLeafletCluster(cluster, position) {
    const _this = this;
    const m = new L.Marker(position, { icon: this.BuildLeafletClusterIcon(cluster) });
    m._leafletClusterBounds = cluster.bounds;
    m.on('click', () => {
      const cbounds = m._leafletClusterBounds;
      let markersArea = _this.Cluster.FindMarkersInArea(cbounds);
      const b = _this.Cluster.ComputeBounds(markersArea);
      if (b) {
        const bounds = new L.LatLngBounds(new L.LatLng(b.minLat, b.maxLng), new L.LatLng(b.maxLat, b.minLng));
        ...
        if (zoomLevelAfter === zoomLevelBefore) {
          ...
          if (filteredBounds.length > 0) {
            ...
            for (i = 0, l = markersArea.length; i < l; ++i) {
              const markerPos = markersArea[i].position;
  ...
  BuildLeafletMarker(marker, position) {
  ...
  ProcessView() {
    ...
    const clusters = this.Cluster.ProcessView({
    ...
    const clusterCreationListPopOne = [];
    const opacityUpdateList = [];
    const workingList = [];
    for (i = 0, l = clusters.length; i < l; ++i) {
    ...
    clusters.forEach((cluster) => {
      ...
      const position = new L.LatLng(cluster.averagePosition.lat, cluster.averagePosition.lng);
      const oldMarker = data._leafletMarker;
      if (oldMarker) {
    ...
    for (i = 0, l = objectsOnMap.length; i < l; ++i) {
      ...
      if (idata._leafletMarker._removeFromMap) {
        ...
        if (marker._zoomLevel === zoom) {
          const pa = icluster.averagePosition;
          ...
          for (j = 0, ll = clusterCreationList.length; j < ll; ++j) {
            ...
            if (marker._population === 1 && jcluster.population === 1 &&
              ...
              const pb = jcluster.averagePosition;
    ...
    for (i = 0, l = clusterCreationList.length; i < l; ++i) {
      ...
      idata = icluster.data;
      const iposition = idata._leafletPosition;
      var creationMarker;
...
var PruneClusterLeafletSpiderfier = (L.Layer ? L.Layer : L.Class).extend({
  ...
  _generatePointsCircle(count, centerPt) {
    let circumference = this.spiderfyDistanceMultiplier * this._circleFootSeparation * (2 + count),
